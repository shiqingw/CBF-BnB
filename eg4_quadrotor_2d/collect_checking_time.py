...
def diagnosis(exp_num):
    ...
    # Step 3a
    filepath = f"{results_dir}/00_dreal_feasibility_result_1.0E-02.pkl"
    if os.path.exists(filepath):
        results = load_dict(filepath)
        all_info.append(int(results["success"]))
        all_info.append(results["time"])
    ...
    # Step 3b
    filepath = f"{results_dir}/00_grid_feasibility_result_linux_cpu_1.0E-02.pkl"
    if os.path.exists(filepath):
        results = load_dict(filepath)
        all_info.append(int(results["success"]))
        all_info.append(results["time"])
    ...
    # Step 3c
    filepath = f"{results_dir}/00_grid_feasibility_result_linux_cuda_1.0E-02.pkl"
    if os.path.exists(filepath):
        results = load_dict(filepath)
        all_info.append(int(results["success"]))
        all_info.append(results["time"])
