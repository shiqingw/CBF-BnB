...
class InclusionAdaptiveMeshSecondOrder(AdaptiveMesh):
    ...
    def bound(self):
        ...
        for i, (x_lb_batch, x_ub_batch) in enumerate(data_loader):
            ...
            # First-order bound
            ## Lower bound
            f_lb_first_order_batch = V_batch.clone()
            ...
            f_lb_first_order_batch[case1_idx] += norm_l2_V_dx_batch[case1_idx]**2 / (2.0 * self.hess_f_l2)
            ...
            f_lb_first_order_batch[case2_idx] += self.mesh_radius_l2 * norm_l2_V_dx_batch[case2_idx] - self.mesh_radius_l2**2 * self.hess_f_l2 / 2.0

            ## Upper bound
            f_ub_first_order_batch = V_batch + self.mesh_radius_l2 * norm_l2_V_dx_batch + self.mesh_radius_l2**2 * self.hess_f_l2 / 2.0
            ...
            f_lb_second_order_batch[case1_idx] += norm_l2_V_dx_batch[case1_idx]**2 / (2.0 * norm_l2_V_dxdx_batch[case1_idx]) - \
            ...
            f_lb_second_order_batch[case2_idx] += self.mesh_radius_l2 * norm_l2_V_dx_batch[case2_idx] - self.mesh_radius_l2**2 * norm_l2_V_dxdx_batch[case2_idx] / 2.0 - \
                self.mesh_radius_l2**2 * self.thrid_order_elementwise_l2 * self.mesh_radius_l1 / 6.0

            ## Upper bound
            f_ub_second_order_batch = V_batch + self.mesh_radius_l2 * norm_l2_V_dx_batch + self.mesh_radius_l2**2 * norm_l2_V_dxdx_batch / 2.0 \
                                        + self.mesh_radius_l2**2 * self.thrid_order_elementwise_l2 * self.mesh_radius_l1 / 6.0
