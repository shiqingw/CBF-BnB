...
class PositiveProbAdaptiveMeshSecondOrder(AdaptiveMesh):
    def __init__(self, f, lip_f_l2, hess_f_l2, thrid_order_elementwise_l2, x_lb, x_ub, batch_size, cutoff_radius, dtype, device):
    ...
    def bound(self):
        ...
        for i, (x_lb_batch, x_ub_batch) in enumerate(data_loader):
            ...
            norm_l2_V_dx_batch = torch.linalg.norm(V_dx_batch, ord=2, dim=1) # shape (N,)
            norm_l2_V_dxdx_batch = torch.linalg.norm(V_dxdx_batch, ord=2, dim=(1,2)) # shape (N,)

            # Zero-order bound
            f_ub_zero_order_batch = V_batch.clone()
            f_lb_zero_order_batch = V_batch - self.lip_f_l2 * self.mesh_radius_l2

            # First-order bound
            ## Upper bound
            f_ub_first_order_batch = V_batch.clone()
            ### Case 1: ||V_dx||_2 <= self.mesh_radius_l2 * self.hess_f_l2
            case1_idx = torch.where(norm_l2_V_dx_batch <= self.mesh_radius_l2 * self.hess_f_l2)[0]
            f_ub_first_order_batch[case1_idx] += - norm_l2_V_dx_batch[case1_idx]**2 / (2.0 * self.hess_f_l2)
            ### Case 2: ||V_dx||_2 > self.mesh_radius_l2 * self.hess_f_l2
            case2_idx = torch.where(norm_l2_V_dx_batch > self.mesh_radius_l2 * self.hess_f_l2)[0]
            f_ub_first_order_batch[case2_idx] += - self.mesh_radius_l2 * norm_l2_V_dx_batch[case2_idx] + self.mesh_radius_l2**2 * self.hess_f_l2 / 2.0

            ## Lower bound
            f_lb_first_order_batch = V_batch - self.mesh_radius_l2 * norm_l2_V_dx_batch - self.mesh_radius_l2**2 * self.hess_f_l2 / 2.0

            # Second-order bound
            ## Upper bound
            f_ub_second_order_batch = V_batch.clone()
            ### Case 1: ||V_dx||_2 <= self.mesh_radius_l2 * ||V_dxdx||_2
            case1_idx = torch.where(norm_l2_V_dx_batch <= self.mesh_radius_l2 * norm_l2_V_dxdx_batch)[0]
            f_ub_second_order_batch[case1_idx] += - norm_l2_V_dx_batch[case1_idx]**2 / (2.0 * norm_l2_V_dxdx_batch[case1_idx]) + \
                self.mesh_radius_l2**2 * self.thrid_order_elementwise_l2 * self.mesh_radius_l1 / 6.0
            ### Case 2: ||V_dx||_2 > self.mesh_radius_l2 * ||V_dxdx||_2
            case2_idx = torch.where(norm_l2_V_dx_batch > self.mesh_radius_l2 * norm_l2_V_dxdx_batch)[0]
            f_ub_second_order_batch[case2_idx] += - self.mesh_radius_l2 * norm_l2_V_dx_batch[case2_idx] + self.mesh_radius_l2**2 * norm_l2_V_dxdx_batch[case2_idx] / 2.0 +\
                self.mesh_radius_l2**2 * self.thrid_order_elementwise_l2 * self.mesh_radius_l1/ 6.0
            
            ## Lower bound
            f_lb_second_order_batch = V_batch - self.mesh_radius_l2 * norm_l2_V_dx_batch - self.mesh_radius_l2**2 * norm_l2_V_dxdx_batch / 2.0 \
                                        - self.mesh_radius_l2**2 * self.thrid_order_elementwise_l2 * self.mesh_radius_l1 / 6.0
            ...
            # Set f_ub_batch to +inf if B(x_m, self.mesh_radius_l2) is not completely outside the cutoff radius and negative
            x_m_l2_batch = torch.linalg.norm(x_m_batch, ord=2, dim=1)
            f_ub_batch[torch.where((x_m_l2_batch - self.mesh_radius_l2 <= self.cutoff_radius) & (f_ub_batch <= 0))] = float("inf")
    ...
    def refine(self, refine_idx):
        ...
        keep_idx = torch.where(xi_max_l2 > self.cutoff_radius)[0]
