...
class StabilityProbAdaptiveMeshFirstOrder(AdaptiveMesh):
    ...
    def bound(self):
        ...
        for i, (x_lb_batch, x_ub_batch) in enumerate(data_loader):
            ...
            dist_delta_first_order_coefs_batch += norm_V_dxdx_G_l2_batch * epsilon_batch # shape (N,)
            ...
            norm_V_dx_f_dx_plus_V_dxdx_f_batch_l2_batch = torch.linalg.norm(V_dx_f_dx_plus_V_dxdx_f_batch, ord=2, dim=1) # shape (N,)
            psi_ub_batch_mixed_first_order += norm_V_dx_f_dx_plus_V_dxdx_f_batch_l2_batch * self.mesh_radius_l2 # shape (N,)
