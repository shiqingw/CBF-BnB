...
class PositiveProbAdaptiveMeshSecondOrder(AdaptiveMesh):
    ...
    def bound(self):
        ...
        for i, (x_lb_batch, x_ub_batch) in enumerate(data_loader):
            ...
            f_ub_second_order_batch += - abs_V_dx_c_vec_batch + self.mesh_radius_l2**2 * self.thrid_order_elementwise_l2 * self.mesh_radius_l1 / 6.0
            sign_V_dx_batch = torch.sign(V_dx_batch) # shape (N, n)
            sign_V_dx_c_vec_batch = sign_V_dx_batch * c_vec_batch # shape (N, n)
            f_ub_second_order_batch += 0.5 * torch.bmm(torch.bmm(sign_V_dx_c_vec_batch.unsqueeze(1), V_dxdx_batch), sign_V_dx_c_vec_batch.unsqueeze(2)).squeeze(2).squeeze(1)
            
            ## Lower bound
            f_lb_second_order_batch = V_batch - abs_V_dx_c_vec_batch - self.mesh_radius_l2**2 * norm_l2_V_dxdx_batch / 2.0 \
