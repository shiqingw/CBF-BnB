...
from dreal import Variable, cos, sin, sqrt, Expression, Config, logical_and, logical_not, logical_imply, CheckSatisfiability
...
if __name__ == '__main__':
    ...
    drift = np.array([0.0, 0.0, 0.0])
    h_dx = np.array([cbf.Differentiate(x1), cbf.Differentiate(x2), cbf.Differentiate(x3)])
    feasibility_condition = np.dot(drift, h_dx)
    actuation = np.array([[cos(x3), 0.0], [sin(x3), 0.0], [0.0, 1.0]]) # (state_dim, control_dim)
