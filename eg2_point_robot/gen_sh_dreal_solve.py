...
def generate_sh_script_dreal_inclusion_solve(filename, exp_nums, dreal_precision):
    with open(filename, "w") as file:
        for exp_num in exp_nums:
            ...
            command2 = f"python -u eg2_point_robot/dreal_inclusion_solve.py --exp_num {exp_num} " + \
                f"--dreal_precision {dreal_precision:.1E} " + \
                f"> eg2_results/{exp_num:03d}/output_dreal_inclusion_solve_{dreal_precision:.1E}.out\n"
            file.write(command1)
            file.write(command2)
...
def generate_sh_script_dreal_feasibility_solve(filename, exp_nums, dreal_precision):
    with open(filename, "w") as file:
        for exp_num in exp_nums:
            command1 = f"mkdir eg2_results/{exp_num:03d}\n"
            command2 = f"python -u eg2_point_robot/dreal_feasibility_solve.py --exp_num {exp_num} " + \
                f"--dreal_precision {dreal_precision:.1E} " + \
                f"> eg2_results/{exp_num:03d}/output_dreal_feasibility_solve_{dreal_precision:.1E}.out\n"
            file.write(command1)
            file.write(command2)
