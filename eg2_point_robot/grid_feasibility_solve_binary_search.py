...
def test_feasibility(beta,
                     system, cbf_nn,
                     lip_cbf_nn, hess_bound_cbf_nn, third_order_elementwise_bound_cbf_nn,
                     test_settings, precision, config, device):
    ...
    disturbance_lower_bound = beta * torch.tensor(disturbance_config["lower_bound"], dtype=config.pt_dtype, device=device)
    disturbance_upper_bound = beta * torch.tensor(disturbance_config["upper_bound"], dtype=config.pt_dtype, device=device)
    ...
    # Adaptive mesh
    n_pieces_each_dim = torch.tensor([2, 2, 2, 2], dtype=torch.long)
    ...
    print(f"> Total time : {format_time(time_checking)} = {time_checking} s")
...
if __name__ == '__main__':
    ...
    # Binary search over beta in [1, 10]
    precision = args.precision
    low, high = 1.0, 10.0
    best_beta = low
    time_binary_search = 0.0
    while high - low > 0.1:
        mid = (low + high) / 2
        print(f"==> Current beta range: [{low:.4f}, {high:.4f}]")
        print(f"==> Testing beta = {mid:.4f}")
        start_time = time.time()
        ok = test_feasibility(beta=mid,
                              system=system, 
                              cbf_nn=cbf_nn,
                              lip_cbf_nn=lip_cbf_nn,
                              hess_bound_cbf_nn=hess_bound_cbf_nn,
                              third_order_elementwise_bound_cbf_nn=third_order_elementwise_bound_cbf_nn,
        ...
        if ok:
            best_beta = mid
            low = mid
            print(f"==> Feasibility test passed for beta = {mid:.4f}. Reducing beta range to [{low:.4f}, {high:.4f}].")
        else:
            high = mid
            print(f"==> Feasibility test failed for beta = {mid:.4f}. Reducing beta range to [{low:.4f}, {high:.4f}].")
        print(f"==> Time for this iteration: {format_time(end_time - start_time)}")
        print(f"==> Total time for binary search so far: {format_time(time_binary_search)} = {time_binary_search} s")
        print("#" * 50)
    print(f"==> Best beta found: {best_beta:.4f}")

    # Optionally save the result
    platform = config.platform
    save_dict({'beta': best_beta}, f"{results_dir}/00_grid_feasibility_result_{platform}_{device}_{precision:.1E}.pkl")
