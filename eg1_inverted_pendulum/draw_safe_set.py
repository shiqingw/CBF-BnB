...
def draw_safe_set(exp_num):
    ...
    save_dir = "{}/eg1_results/{:03d}".format(str(Path(__file__).parent.parent), 0)
    ...
    X_unsafe_lb, X_unsafe_ub = decompose_hyperrectangle(state_lower_bound, state_upper_bound, 
    ...
    for (x_idx, y_idx) in pairwise_idx:
        ...
        ax.legend(handles=[legend_safe_set,legend_infeasibility, legend_inadmissible],
                  loc='upper right',
                  fontsize=legend_fontsize)


        # # Create proxy artists for legend
        # proxy_safe_set = mlines.Line2D([], [], color='tab:blue', linewidth=6, label=r'$h_{\theta}(x) \geq 0$', linestyle='solid')
        # proxy_infeasibility = mlines.Line2D([], [], color='tab:red', linewidth=6, label=r'$\tilde{H}(x) < 0$', linestyle='solid')
        # ax.legend(handles=[proxy_safe_set, proxy_infeasibility], loc='upper right', fontsize=legend_fontsize)
