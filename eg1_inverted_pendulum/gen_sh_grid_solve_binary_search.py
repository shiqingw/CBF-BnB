...
def generate_sh_script_grid_stability_solve(filename, exp_nums, device, precision):
    with open(filename, "w") as file:
        for exp_num in exp_nums:
            ...
            command2 = f"python -u eg1_inverted_pendulum/grid_feasibility_solve_binary_search.py " + \
                f"--exp_num {exp_num} " +\
                f"--device {device} " +\
                f"--precision {precision:.1E} " +\
                f"> eg1_results/{exp_num:03d}/output_grid_feasibility_solve_binary_search_{device}_{precision:.1E}.out\n"
            file.write(command1)
            file.write(command2)
