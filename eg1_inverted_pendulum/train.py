...
if __name__ == '__main__':
    ...
    safe_set_lower_bound = np.array(safe_set_config["safe_set_lower_bound"], dtype=config.np_dtype)
    safe_set_upper_bound = np.array(safe_set_config["safe_set_upper_bound"], dtype=config.np_dtype)
    is_safe = np.all((state_np >= safe_set_lower_bound) & (state_np <= safe_set_upper_bound), axis=1)
    label_np = np.where(is_safe, 1, -1)
    del is_safe
    ...
    is_safe = np.all((state_flatten_np >= safe_set_lower_bound) & (state_flatten_np <= safe_set_upper_bound), axis=1)
    label_np = np.where(is_safe, 1, -1)
    del meshgrid, is_safe
    ...
    def is_safe_fun(x):
        is_safe = np.all((x >= safe_set_lower_bound) & (x <= safe_set_upper_bound), axis=1)
        label_np = np.where(is_safe, 1, -1)
        return label_np
