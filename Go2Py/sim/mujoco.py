...
class Go2Sim:
    def __init__(self, 
        ...
        if render:
            ...
            # Option 1: Set the camera to a certain named camera in the model
            # viewer_cam_id = mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_CAMERA, "track_camera")
            # if viewer_cam_id < 0:
            #     raise ValueError("Camera not found in the model.")
            # self.viewer.cam.type = mujoco.mjtCamera.mjCAMERA_FIXED
            # self.viewer.cam.fixedcamid = viewer_cam_id

            # Option 2: Set the camera to a certain position and orientation
            self.viewer.cam.distance = 5.0
            self.viewer.cam.azimuth = 90
            self.viewer.cam.elevation = -45
            self.viewer.cam.lookat[:] = np.array([0.0, -0.25, 0.824])
            self.moving_viewer_camera = moving_viewer_camera
        else:
            self.render = False
    ...
    def updateHeightMap(self, height_map, hfield_size = (300,300), raw_depth_to_height_ratio = 255.):
        # try:
        # map = cv2.resize(height_map, hfield_size)/raw_depth_to_height_ratio
        # self.height_map = np.flip(map, axis=0).reshape(-1)
        self.model.hfield_data = height_map.flatten()
        # if self.render:
    ...
    def getLaserScan(self, nray=1000, max_range=30):

        lidar_angles = np.linspace(0.0, 2 * np.pi, nray).reshape(-1, 1)
        vec_in_body = np.zeros((nray, 3), np.float64)
        vec_in_body[:, 0] = np.cos(lidar_angles).flatten()
        vec_in_body[:, 1] = np.sin(lidar_angles).flatten()
        ...
        pnt = t.copy()
        pnt[2] += 0.25
        vec_in_w = vec_in_body @ world_R_body.T

        # floor has group 0. height_map has group 1. The following ignores the floor
        # to deal with the pitch of the robot.
        mujoco.mj_multiRay(
            m=self.model,
            d=self.data,
            pnt=pnt,
        ...
        dist[dist == -1.0] = max_range
        dist = np.clip(dist, 0, max_range)
        pcd = dist.reshape(-1, 1) * vec_in_body
        return pcd
    ...
    def add_visual_capsule(self, point1, point2, radius, rgba, id_geom_offset=0, limit_num=False):
        ...
        mujoco.mjv_connector(scene.geoms[id_geom],
